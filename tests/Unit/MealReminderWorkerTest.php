...
it('schedules Android meal reminders while the device is idle', function (): void {
    ...
    expect($manifest)
        ...
        ->toContain('"meal-reminder:check --meal=$mealId --date=$localDate --result=${resultFile.absolutePath}"')
        ->toContain('resultFile.readText()')
        ->toContain('MEAL_DUE_PREFIX + mealId -> showMealReminder')
        ->toContain('MEAL_LOGGED_PREFIX + mealId -> Unit')
        ->not->toContain('MEAL_REMINDER_MAX_RETRIES')
        ->not->toContain('output.contains(MEAL_DUE_PREFIX + mealId)')
