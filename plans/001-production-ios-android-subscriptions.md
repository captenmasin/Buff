...
> **Execution checkpoint (2026-09-01)**: The source implementation is complete
...
- **Status**: IN PROGRESS — source complete; Plan 003 launch work remains
- **Priority**: P1
...
- [x] Confirm the production API hostname. `https://api.usebuff.app` resolves
  and returned HTTP 200 on 2026-09-01.
...
- [ ] One RevenueCat entitlement named `buff_plus` maps the approved
...
- [ ] API refresh and webhook jobs query RevenueCat; neither trusts client or
  webhook lifecycle fields as entitlement truth. Sandbox projection is allowed
  only behind the controlled review flag, which is false after launch.
- [ ] Webhooks require both configured Authorization and valid five-minute HMAC
  signatures over raw bytes, acknowledge valid deliveries with HTTP 200, use no
  route throttle, and queue idempotent refresh.
