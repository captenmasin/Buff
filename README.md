...
- Confirm the final iOS archive includes both HealthKit purpose strings; Apple upload validation requires `NSHealthUpdateUsageDescription` even though Buff only reads workouts.
...
7. Explain that Buff reads workouts/exercise calories, may sync them to the Buff account/API, writes nothing to HealthKit, never uses health data for advertising, and remains usable when HealthKit permission is refused. `NSHealthShareUsageDescription` describes read access. Apple upload validation also requires `NSHealthUpdateUsageDescription` in the final app bundle; its text must accurately state that Buff does not write to Apple Health. Review [HealthKit privacy](https://developer.apple.com/documentation/healthkit/protecting-user-privacy).
